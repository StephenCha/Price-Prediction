...
class TemporalEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        year_x = self.year_embed(x[:, :, 1])
        day_x = self.day_embed(x[:, :, 3])
        month_x = self.month_embed(x[:, :, 2])

        return weekday_x + day_x + month_x + year_x
...
class DataEmbedding(nn.Module):
    ...
    def forward(self, x, x_mark):
        ...
        temporal = self.temporal_embedding(x_mark)
        # print(value.shape, position.shape, temporal.shape)
        x = value + position + temporal
